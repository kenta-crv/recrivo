...
RSpec.describe InterviewEngine::ResponseEvaluator do
  ...
  describe '#evaluate' do
    context 'テストモード（APIキー無し）' do
      ...
      it '内容に応じたオフライン評価をする（固定82点ではない）' do
        ...
        expect(result.ai_reasoning).to include('Offline heuristic')
        expect(result.final_score).to be < 82
      end
    end

    context 'TEST_MODEでもAPIキーがあればLLM評価' do
      ...
      it 'LLMを呼ぶ' do
        ...
        result = evaluator.evaluate
        expect(result.ai_reasoning).to eq('薄い回答')
        expect(result.final_score).to eq((40 * 0.4 + 35 * 0.4 + 30 * 0.2).round(2))
      end
    end
